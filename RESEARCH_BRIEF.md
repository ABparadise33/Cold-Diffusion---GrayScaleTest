...
## 2026-09-16 — Mixed50k stage monitoring and preview-failure recovery

- Authorized scope: support comparison at10k/20k/30k/40k/50k for mixed training; handle user-reported asynchronous CUDA launch failure in full-scene preview. No GPU training or root-cause claim from local tests.
- Smallest test: unchanged model/optimizer/data/degradation, log fixed128px center crops (up to8/domain/split), source/output hashes and RGB/Lab values. Keep existing50-step training-loss averages and full combined validation; save every10k milestone. Synthetic CPU tests compare fixed identities/RNG, checkpoint state and stage report integrity.
- Failure control: persist completed-validation checkpoint and fixed crops before full-scene preview. Log phase/failure and rethrow non-OOM errors; explicit full-scene-disable flag and CUDA_LAUNCH_BLOCKING for localization, never reinterpret unspecified failure as OOM. Known prior mixed hash migration only; reject other source/training/data changes.
- Evaluation: separate small UIEB/DIV2K train/val diagnostic curves and image strips; full combined validation retains best checkpoint. Small subsets do not establish whole-domain gains. Strict plot identity checks prevent different inputs/partial retries being joined.
- Budget/decision: user-selected0→50k run, unchanged saturation1.0 and128px crops. Compare each10k and fixed1k records before further training/saturation experiments; no automatic new run or early stopper. GPU fault remains unresolved pending synchronous trace/environment evidence.
